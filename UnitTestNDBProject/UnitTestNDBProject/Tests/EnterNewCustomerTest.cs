...
namespace UnitTestNDBProject.Tests
{
    ...
    //            _EnterNewCustomerPage.EnterAddressLine1(addressLine1).EnterAddressLine2(addressLine2).EnterCity(city).SelectState(state).EnterZip(zipCode);

    //            if (counter < newCustomerData.Addresses.Count - 1)
    //            {
    //                _EnterNewCustomerPage.ClickOnAddAddressPlusButton();
    //            }
    ...
    //        {
    //            _EnterNewCustomerPage.ClickTaxExemptionCheckBox();
    ...
    //                _EnterNewCustomerPage.EnterTaxIDNumber(taxId, counter + 1).SelectTaxState(taxState, counter + 1).ClickDoesntExpireCheckBox(counter + 1);

    //                if (counter < newCustomerData.TaxNumbers.Count - 1)
    //                {
    //                    _EnterNewCustomerPage.AddTax();
    //                }
