...
namespace UnitTestNDBProject.Tests
{
    ...
    class EnterNewCustomerTest : BaseTestClass
    {
        ...
        public void A3_VerifyCustomerCreation()
        {
            ...
            EnterNewCustomerPage_.ClickEnterNewCustomerButton().EnterFirstName(sheetData.firstName).EnterLastName(sheetData.lastName).EnterPhoneNumber(sheetData.phoneNumber).SelectPhoneType(sheetData.phoneType);

            EnterNewCustomerPage_.ClickOnAddressLine1().ContinueNewCustomerCreation().EnterAddressLine1(sheetData.addressline1).EnterCity(sheetData.city).SelectState(sheetData.state).enterZip(sheetData.zipcode);
